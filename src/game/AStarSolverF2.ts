    ...
    private has2x2Deadlock(boxSet: Uint8Array): boolean {
        for (let y = 0; y < this.H - 1; y++) {
            for (let x = 0; x < this.W - 1; x++) {
                const tl = y * this.W + x, tr = tl + 1;
                const bl = tl + this.W,    br = bl + 1;
        ...
        while (heap.size > 0) {
            ...
            for (const boxCell of current.boxes) {
                ...
                for (let d = 0; d < 4; d++) {
                    ...
                    if (this.has2x2Deadlock(stateBoxSet)) {
                    ...
                    if (this.hasFrozenDeadlock(nextBoxes, stateBoxSet)) {
                        stateBoxSet[boxCell] = 1; stateBoxSet[destCell] = 0; continue;
                    }
                    ...
                    heap.push(nextG + this.weight * nextH, {
                        boxes: nextBoxes,
